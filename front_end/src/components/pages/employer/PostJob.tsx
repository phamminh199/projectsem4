...
import { getSession,getUserSignIn } from '../../share/sharedFunction';
...
console.log('---------------------------------------------');
   ...
   const handleChange = (e: any) => {
      ...
      if(e.target.name =='salary'){
         ...
         datePostMin.setDate(datePostMin.getDate() + 0);
   ...
   const postData = async (data: FormAddType) => {
      ...
      let response: any;
      try {
         response = await collectionAPI.postJob(data);
   ...
   const postData_postJob = async (data: JobObjFormat) => {
      console.log('data: ', data);
      let response: any;
      try {
         response = await collectionAPI.postJob(data);
         console.log("response: " + JSON.stringify(response, null, 4));
         console.log('postData_postJob2');
      // onReset();
         // localStorage.setItem('activeComponent', 'Cars.tsx')
         // alert("Succesfull post new job !");
         // window.location.reload();
   ...
   const handleSubmitAdd = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const userSignIn = getUserSignIn();
      const address = groupState.address+', '+nameWardSelected+', '+nameDistrictSelected
      const min = 50000;
      const max = 100000;

      const randomNumber = Math.floor(Math.random() * (max - min + 1)) + min;
      await postData_postJob(
         {
            idjob: randomNumber,
            idemployer: userSignIn.idemployer,
            idcontroller: 1,
   ...
   return (
      ...
      <div className={clsx(styles.component_PostJob )}>
