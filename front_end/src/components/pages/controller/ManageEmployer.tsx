...
// sdfjhLKhl349(*&)
   ...
   const handleSubmitAdd = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      let response: any;
      try {
         response = await collectionAPI.addEmployer(data);
...
// -------------------------------------------------------edit controller start
   ...
   const onEdit = (idemployer: number) => {
      setStateBackShadow(true);
      setStateViewDialogEdit(true);
      const obj = stateArrAllFinal.find((item) => item.idemployer === idemployer);
      setGroupState(obj);
      console.log('obj: ', obj);
   }
   const handleSubmitEdit = async (e: React.FormEvent<HTMLFormElement>) => {
     ...
     // validate
      ...
      let data = 
         {
            idemployer: stateUserSignIn.idemployer,
            idcompany: 1, //cái này phải bắt theo cái obj đc edit cho đúng, check lại
            email: groupState.email,
            password: passwordEncrypted,
            fullname: groupState.fullname,
            title: groupState.title,
      ...
      let response: any;
      try {
         response = await collectionAPI.editEmployer(data);
...
// -------------------------------------------------------sort end
   ...
   const getData = async () => {
      try {
         const response: any = await collectionAPI.findAllEmployer(); //phải có await nghĩa là khi nào có data rồi thì mới lấy
         setStateArrAll(response.data);
         setStateArrAllFinal(response.data);
   ...
   console.log('stateArrAllFinal: ', stateArrAllFinal);

   const SwitchStatusUser = async (idemployer: number) => {
      const data = stateArrAllFinal.find((item) => item.idemployer === idemployer);
   ...
   const onView = (idemployer: number) => {
      setStateIsViewDetailsAppear(true);
      setStateBackShadow(true);
      // console.log("idemployer: " + idemployer);
      // console.log(stateArrAll);
      const obj = stateArrAll.find((item) => item.idemployer === idemployer); //lấy một object đầu tiên được tìm thấy mà thoả điều kiện
      console.log('obj: ', obj);
      // console.log(obj);
   ...
   useEffect(() => {
      getData();
      const obj = getSession('sessionemployersignin');
   ...
   return (
      <div className={clsx(styles.component_ManageEmployer)}>
         <div className={clsx(styles.main)}>
            ...
            <div className={clsx(styles.container)} >
               <div className={clsx(styles.printPDF)} id="pdfAll">
                  ...
                  <div className={clsx(styles.viewWrapper)}>
                     {
                        stateArrAllFinal.slice(stateFrom, stateTo).map((obj, index) => {
                           return ( 
                              <div className={clsx(styles.row)} key={obj.idemployer}>
                                 <p>{obj.fullname}</p>
                                 <p>{obj.email}</p>
                                 <p>{obj.phone}</p>
                                 ...
                                 <div className={clsx(styles.actionWrapper)}>
                                    {(() => {
                                          if (obj.status === "enable") {
                                             return (
                                                <button onClick={()=>SwitchStatusUser(obj.idemployer)} className={clsx(styles.btnDisable)}>Disable</button>
                                             )
                                          } else if (obj.status === "disable") {
                                             return (
                                                <button onClick={()=>SwitchStatusUser(obj.idemployer)} className={clsx(styles.btnActivate)}>activate</button>
                                             )
                                          }
                                 
                                    })()}
                                    <button onClick={()=>onView(obj.idemployer)} className={clsx(styles.btnView)}>View</button>
                                 </div> 
                              </div>
                  ...
                  {/* {(() => {
                     if (stateUserSignIn.idemployer === obj.idemployer) {
                        return (
                           <>
                              <button onClick={()=>onEdit(obj.idemployer)} className={clsx(styles.btnEdit)}>Edit</button>
                           </>
                        )
                     }
                  })()} */}
   ...
   {/* --------------------------------------------------------------- */}
               <div className={clsx(styles.row)}>
                  ...
                  <input type="text" name="title" value={groupState.title} onChange={handleChange}  required/>

               </div>
               {groupStateError.title === "error" && 
               <div className={clsx(styles.row, styles.error)}>
                  <label></label>
                  <span className={clsx(styles.right)}>
                     <span>Title can not leave blank and has at least 5 letters and maximum 50 letters</span>
                  </span>
               </div>
               }
