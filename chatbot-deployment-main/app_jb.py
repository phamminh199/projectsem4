...
from chat_jb import get_response
...
model = pickle.load(open("D:\CODE_LOCAL\PYTHON\MACHINE_LEARNING\chatbot-deployment-main\modelSalary.pkl", "rb"))
...
@app.route('/api/chatbotaddsearchmonitor', methods=['POST'])
def search_handler():
...
@app.route('/api/chatbotFindallsearchmonitor', methods=['GET'])
def get_searches():
    conn = pyodbc.connect('DRIVER={SQL Server};SERVER=LAPTOPVUNAACER\\SQLEXPRESS;DATABASE=chatbotDB;UID=sa;PWD=123')
    cursor = conn.cursor()
...
# Load the pickled model
modelSalary = pickle.load(open("D:\CODE_LOCAL\PYTHON\MACHINE_LEARNING\chatbot-deployment-main\modelSalary.pkl", "rb"))

# localhost:5000/api/predictSalary
@app.route('/api/predictSalary', methods=['POST'])
def predictSalary():
    data = request.get_json()  # Get the JSON data from the request
    features = [data[col] for col in data.keys()]  # Extract the features from the JSON data
    ...
    prediction = modelSalary.predict(input_features)
